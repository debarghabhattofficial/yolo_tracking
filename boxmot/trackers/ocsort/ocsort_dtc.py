...
class KalmanBoxTracker(object):
    ...
    @staticmethod
    def multi_gmc_dtc(stracks, H=np.eye(2, 3), img=None, debug=False):
    ...
    @staticmethod
    def multi_gmc_lo_dtc(stracks, H=np.eye(2, 3), img=None, debug=False):
        # NOTE: Make changes to this method to account for
        # camera motion compensation while updating the
        # state of the tracklets.
        if len(stracks) > 0:
...
class OCSORT_DTC(object):
    ...
    def update(self, dets, img):
        ...
        KalmanBoxTracker.multi_gmc_dtc(self.trackers, warp, img, self.debug)
        KalmanBoxTracker.multi_gmc_lo_dtc(self.trackers, warp, img, self.debug)
