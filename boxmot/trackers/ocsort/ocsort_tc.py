...
from boxmot.motion.kalman_filters.ocsort_tc_kf import KalmanFilter
from boxmot.utils.association import associate_tc, linear_assignment
...
def convert_bbox_to_z_tc(bbox):
    """
    Takes a bounding box in the form 
    [u1, v1, u2, v2, conf] 
    and  returns z in the form [u1, v1, u2, v2] 
    ...
    u2 = bbox[2]
    v2 = bbox[3]
    z = np.array([u1, v1, u2, v2]).reshape((4, 1))
    return z


# @nb.njit(fastmath=True, cache=True)
def convert_x_to_bbox_tc(x, score=None):
    """
    Takes a bounding box in the tlbr form 
    [u1, v1, u2, v2] 
    and returns it in the form [u1, v1, u2, v2] where 
...
# with speed_direction_tc() method.
# @nb.njit(fastmath=True, cache=True)
def speed_direction_tc(bbox1, bbox2):
    # Center oordinates for bbox1 centre
    cx1 = (bbox1[0] + bbox1[2]) / 2.0
    cy1 = (bbox1[1] + bbox1[3]) / 2.0

    # Center coordinates for bbox2 centre
...
class KalmanBoxTracker(object):
    ...
    def __init__(self, bbox, cls, det_ind, delta_t=3):
        ...
        self.kf.F = np.array(
            [
                [1, 0, 0, 0, 1, 0, 0, 0],  # tl u1
                [0, 1, 0, 0, 0, 1, 0, 0],  # tl v1
                [0, 0, 1, 0, 0, 0, 1, 0],  # br u2
                [0, 0, 0, 1, 0, 0, 0, 1],  # br v2
                [0, 0, 0, 0, 1, 0, 0, 0],  # velocity tl du1
                [0, 0, 0, 0, 0, 1, 0, 0],  # velocity tl dv1
                [0, 0, 0, 0, 0, 0, 1, 0],  # velocity br du2
                [0, 0, 0, 0, 0, 0, 0, 1],  # velocity br dv2
            ]
        )
        ...
        self.kf.H = np.array(
            [
                [1, 0, 0, 0, 0, 0, 0, 0],  # tl u1
                [0, 1, 0, 0, 0, 0, 0, 0],  # tl v1
                [0, 0, 1, 0, 0, 0, 0, 0],  # br u2
                [0, 0, 0, 1, 0, 0, 0, 0],  # br v2
            ]
        )
        ...
        # State covariance P
        self.kf.P[4:, 4:] *= 1000.0  # Increase the uncertainty for the unobservable initial velocities.
        self.kf.P *= 10.0

        # Process noise covariance Q
        # self.kf.Q[-1, -1] *= 0.01  # Lower the uncertainty in process's scale velocity.
        self.kf.Q[4:, 4:] *= 0.01  # Lower the uncertainty all velocities of the process function.

        # Initial state [u1, v1, u2, v2, d, du1, dv1, du2, dv2]
        self.kf.x[:4] =  convert_bbox_to_z_tc(bbox)  # Convert bounding box to state variables.
    ...
    def update(self, bbox, cls, det_ind):
        ...
        if bbox is not None:
            ...
            if self.last_observation.sum() >= 0:  # no previous observation
                ...
                self.velocity = speed_direction_tc(
                    bbox1=previous_box, 
                    bbox2=bbox
                )
            ...
            self.hits += 1
            self.hit_streak += 1
            self.kf.update(z=convert_bbox_to_z_tc(bbox)) 
        else:
    ...
    def predict(self):
        ...
        self.history.append(convert_x_to_bbox_tc(x=self.kf.x))
...
class OCSORT_TC(object):
    ...
    def update(self, dets, img):
        ...
        err_msg = f"Unsupported 'dets' 2nd dimension length '{dets.shape[1]}', valid lenghts is 6. It is '{dets}'"
        assert (
            dets.shape[1] == 6
        ), err_msg
        ...
        # Depth of bbox centre not inserted in index 4.
        # Confidence values are available at index 4.
        confs = dets[:, 4]
        ...
        # Depth of bbox centre not inserted in index 4.
        # Therefore, size along axis=1 remains 5.
        ...
        """
        # NOTE: Original OC-SORT + RGBD code called associate_tc() 
        # method instead of associate_tc() method.
        matched, unmatched_dets, unmatched_trks = associate_tc(
            detections=dets[:, 0:5], 
            trackers=trks, 
            ...
            if iou_left.max() > self.asso_threshold:
                ...
                for m in matched_indices:
                    ...
                    self.trackers[trk_ind].update(
                        bbox=dets_second[det_ind, :5], 
                        cls=dets_second[det_ind, 5], 
                        det_ind=dets_second[det_ind, 6]
                    )
