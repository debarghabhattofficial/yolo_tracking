...
from boxmot.motion.kalman_filters.botsort_tc_kf import KalmanFilter
from boxmot.trackers.botsort.basetrack import BaseTrack, TrackState
from boxmot.utils.matching import (
    fuse_score_tc, 
    iou_distance_tc, 
    linear_assignment_tc
)
...
class STrack(BaseTrack):
    ...
    def __init__(self, det, feat=None, feat_history=50):
        # wait activate
        self.xyxy = det[0:4].copy()  # (x1, y1, x2, y2)
        self.score = det[4]
        self.cls = det[5]
        self.det_ind = det[6]
    ...
    def xyxy_tc(self):
        ...
        if self.mean is None:
            ret = self.xyxy.copy()  # (u1, v1, u2, v2)
        else:
            ret = self.mean[:4].copy()  # kf (u1, v1, u2, v2)
        return ret
...
class BoTSORT_TC(object):
    ...
    def update(self, dets, img):
        ...
        err_msg = f"Unsupported 'dets' 2nd dimension length '{dets.shape[1]}', valid lenghts is 6."
        assert (
            dets.shape[1] == 6
        ), err_msg
        ...
        # NOTE: Depth of bbox centre not inserted in index 4. 
        ...
        STrack.multi_gmc_tc(strack_pool, warp, img, self.debug)
        STrack.multi_gmc_tc(unconfirmed, warp, img, self.debug)

        # Associate with high score detection boxes.
        ious_dists = iou_distance_tc(
            atracks=strack_pool, 
            btracks=detections
        )
        if self.fuse_first_associate:
            ious_dists = fuse_score_tc(
        ...
        matches, u_track, u_detection = linear_assignment_tc(
            cost_matrix=dists, 
        ...
        dists = iou_distance_tc(
            atracks=r_tracked_stracks, 
            btracks=detections_second
        )
        matches, u_track, \
            u_detection_second = linear_assignment_tc(
                cost_matrix=dists, 
                thresh=0.5
            )
        if matches.shape[0] > 0:
        ...
        matches, u_unconfirmed, \
            u_detection = linear_assignment_tc(
                cost_matrix=dists, 
                thresh=0.7
            )
        if matches.shape[0] > 0:
        ...
        for t in output_stracks:
            output = []
            output.extend(t.xyxy_tc)  # (x1, y1, x2, y2)
...
def remove_duplicate_stracks(stracksa, stracksb):
    pdist = iou_distance_tc(stracksa, stracksb)
